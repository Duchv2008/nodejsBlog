    ...
    user => {
      if (user.comparePassword(password)) {
        ...
        Token.findOneAndUpdate(
          { user_id: user.id },
          { $set: { hash_token: hash_token, user_id: user.id } },
  ...
  if (req.headers) {
    jwt.verify(refresh_token, 'this is refresh token', function(err, payload) {
      if (err) {
        ...
        if (jwtToken) {
          let payloadAccessToken = jwt.decode(jwtToken, { complete: true })
            .payload;

          if (
            payloadAccessToken.id == userId &&
            refreshTokenCreatedAt == payloadAccessToken.created_at
          ) {
            // find
